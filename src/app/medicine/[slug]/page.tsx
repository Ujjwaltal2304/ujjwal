import { notFound } from "next/navigation";
import { medicineData } from "@/data/siteContent";
import DynamicPageTemplate from "@/components/DynamicPageTemplate";
import { Metadata } from "next";

export function generateStaticParams() {
  return medicineData.map((item) => ({
    slug: item.slug,
  }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const data = medicineData.find((t) => t.slug === slug);
  if (!data) return { title: "Medicine Not Found" };
  
  return {
    title: `${data.title} | Ayurveda Medicine | Dr. Demo Clinic`,
    description: data.description,
  };
}

export default async function MedicinePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const data = medicineData.find((t) => t.slug === slug);
  
  if (!data) {
    notFound();
  }

  return <DynamicPageTemplate data={data} category="Ayurveda Medicine" />;
}
